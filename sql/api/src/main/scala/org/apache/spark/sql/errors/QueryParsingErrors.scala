...
private[sql] object QueryParsingErrors extends DataTypeErrorsBase {
  ...
  def identityColumnIllegalStep(ctx: IdentitySpecContext): Throwable = {
    new ParseException("IDENTITY_COLUMNS_ILLEGAL_STEP", ctx)
  }

  def identityColumnDuplicatedDescriptor(
                                 ctx: IdentitySpecContext, option: String): Throwable = {
    new ParseException(
