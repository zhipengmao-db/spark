...
import org.apache.spark.sql.catalyst.parser.SqlBaseParser.{GeneratedColumnContext, IdentityColumnContext, _}
...
/**
  ...
  override def visitColDefinition(
    ...
    ColumnDefinition(
      ...
      identitySpec = generationExpression.collect {
  ...
  protected def parseIdentityColumn(
    ...
    val (start, step) = visitIdentitySpec(ctx.identitySpec())

    IdentityColumnSpec(start, step, allowExplicitInsert)
  }

  override def visitIdentitySpec(ctx: IdentitySpecContext): (Long, Long) = withOrigin(ctx) {
    val defaultStart = 1
    val defaultStep = 1
    if (ctx == null) {
    ...
    ctx.identitySpecOption().asScala.foreach { option =>
      if (option.start != null) {
        if (start.isDefined) {
          throw QueryParsingErrors.identityColumnDuplicatedDescriptor(ctx, "START")
        }
        start = Some(option.start.getText.toLong)
      } else if (option.step != null) {
        if (step.isDefined) {
          throw QueryParsingErrors.identityColumnDuplicatedDescriptor(ctx, "STEP")
        }
