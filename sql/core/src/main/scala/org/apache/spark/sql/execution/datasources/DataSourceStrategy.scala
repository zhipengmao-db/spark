...
import org.apache.spark.sql.catalyst.util.{GeneratedColumn, IdentityColumnUtil, ResolveDefaultColumns, V2ExpressionBuilder}
...
object DataSourceAnalysis extends Rule[LogicalPlan] {
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
    case CreateTable(tableDesc, mode, None) if DDLUtils.isDatasourceTable(tableDesc) =>
      ...
      if (IdentityColumnUtil.hasIdentityColumns(newSchema)) {
        throw QueryCompilationErrors.unsupportedTableOperationError(
